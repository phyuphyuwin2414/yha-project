      ...
      <section class="attendance">
        <div class="attendance-list">
          ...
          <input style="margin-left:40px;width:60px;background:#ff6b01;color:white;" type="button" class="btn" value="<?php 
            ...
            $sql = "SELECT COUNT(attendid) FROM attendance WHERE attend='Present'";
          ...
          <input style="margin-left:40px;width:60px;background:#ff6b01;color:white;" type="button" class="btn" value="<?php 
            include("inc/dbcon.php");
            $sql = "SELECT COUNT(attendid) FROM attendance WHERE attend='absent'";
            $res = mysqli_query($con,$sql);
            $row = implode(mysqli_fetch_assoc($res));
            echo $row;
          ?>">
          ...
          <span style="width:200px;">Total Leave:</span>
            <input style="width:60px;background:#ff6b01;color:white;margin-left:190px;" type="button" class="btn" value="<?php 
              include("inc/dbcon.php");
              $sql = "SELECT COUNT(attendid) FROM attendance WHERE attend='leave'";
              $res = mysqli_query($con,$sql);
              $row = implode(mysqli_fetch_assoc($res));
              echo $row;
            ?>"><br><br>
          ...
          <table class="table" id="attend">
            ...
            ?>
                ...
                <?php endwhile; }else{
                    ...
                    $sql = "SELECT a.*,c.coursename,s.sectionname,se.timename FROM attendance AS a LEFT JOIN course AS c ON a.courseid=c.courseid LEFT JOIN section AS s ON a.sectionid=s.sectionid LEFT JOIN sectiontime AS se ON a.timeid=se.timeid ORDER BY attendid DESC";
                    $res = mysqli_query($con,$sql);
                    $state=true;
                    while($row=mysqli_fetch_assoc($res)){
                ?>
                    <tr>
                        <td><?php echo $row['attendid']; ?></td>
